...
import { CEBU_CENTER, AMENITIES, PROPERTIES, TRANSIT_DATA } from '../../lib/constants';
...
export const VerityMap = () => {
  ...
  const embeddedOrgId = queryParams.get('org');
  ...
  if (layoutParam) {
      try {
          const parsed = JSON.parse(decodeURIComponent(layoutParam));
  ...
  const visibleProperties = useMemo(() => {
    ...
    if (embeddedOrgId) { filteredProps = PROPERTIES.filter((p, index) => index % 2 === 0); } 
    // REMOVED THE !isEmbedded CHECK HERE to allow search filter in iframe
    return filteredProps.filter(prop => {
        const dist = L.latLng(searchPos).distanceTo(prop.position) / 1000; 
        return dist <= searchRadius;
    });
  }, [viewMode, selectedProperty, searchPos, searchRadius, embeddedOrgId]);

  const visibleAmenities = useMemo(() => {
    if (viewMode !== 'property' || !selectedProperty || isTransitMode) return [];
    let allPins = activeFilter ? (AMENITIES[activeFilter] || []) : Object.values(AMENITIES).flat();
    return allPins.filter(pin => L.latLng(selectedProperty.position).distanceTo(pin.position) / 1000 <= 5);
  }, [viewMode, selectedProperty, isTransitMode, activeFilter]);
  ...
  const handleAmenityClick = async (e, pinId, pin, selectedProp) => {
    ...
    if (routeData) {
        setRouteCache(prev => ({ ...prev, [pinId]: routeData }));
        setPopupInfo(prev => ({ position: e.latlng, name: pin.name, stats: routeData }));
    }
  };
  ...
  const handleLineClick = (e, pinId, pinData, routeStats) => { L.DomEvent.stopPropagation(e); setSelectedPin(pinId); setPopupInfo({ position: e.latlng, name: pinData.name, stats: routeStats }); };
    ...
    <div className="relative w-full h-full overflow-hidden bg-gray-50/50">
      ...
      >
          <MapContainer center={CEBU_CENTER} zoom={13} scrollWheelZoom={true} className="w-full h-full z-0 outline-none" zoomControl={false} >
            ...
            {/* ENABLED SEARCH CONTROLLER IN IFRAME MODE (Removed !isEmbedded) */}
            {viewMode === 'discovery' && (
                <SearchController position={searchPos} setPosition={setSearchPos} radius={searchRadius} isLocked={isPinLocked} setIsLocked={setIsPinLocked} />
            )}
      ...
      {/* LAYER 2: CHIPS WIDGET */}
      <div 
        style={{ ...getStyle('chips'), pointerEvents: 'auto' }} 
        className="flex items-center justify-center z-20"
      >
        {viewMode === 'property' && (
             <IntentChips 
                activeFilter={activeFilter} 
                onFilterChange={(f) => { setActiveFilter(f); setSelectedPin(null); setPopupInfo(null); }} 
                className="!absolute !bottom-auto !left-auto !transform-none w-full h-full flex items-center justify-center gap-2" 
            />
        )}
      </div>

      {/* LAYER 3: SIDEBAR WIDGET */}
      {viewMode === 'property' && (
          <div 
            style={{ ...getStyle('sidebar'), pointerEvents: 'auto' }} 
            className="shadow-2xl z-[30]"
          >
             <PropertySidebar 
                isOpen={true} 
                onClose={handleSidebarClose} 
                onInquire={() => { setCustomSubject(null); setIsModalOpen(true); }}
                activeFilter={activeFilter}
                propertyData={selectedProperty} 
                className="!absolute !inset-0 !w-full !h-full !transform-none rounded-xl overflow-hidden"
             />
          </div>
      )}
      ...
      {/* ENABLED DISCOVERY BANNER IN IFRAME MODE (Removed !isEmbedded) */}
      {viewMode === 'discovery' && (
