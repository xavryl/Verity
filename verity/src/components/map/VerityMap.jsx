...
import { TrafficWidget } from '../widget/TrafficWidget'; // [NEW] Imported from your specified path
...
// --- 1. ICON MAPPING (Verified) ---
const AMENITY_ICONS = {
    'police station': 'police station.svg',
    'police': 'police station.svg',
    'barangay hall': 'barangay hall.svg',
    'barangay': 'barangay hall.svg',
    'k-12': 'basic.svg',
    'basic': 'basic.svg',
    'school': 'basic.svg',
    'college': 'college.svg',
    'university': 'college.svg',
    'hospital': 'hospital.svg',
    'clinic': 'clinic.svg',
    'dental clinic': 'dental clinic.svg',
    'diagnostic center': 'diagnostic center.svg',
    'public market': 'public market.svg',
    'market': 'public market.svg',
    'bank': 'bank.svg',
    'blood bank': 'blood bank.svg',
    'bus': 'bus.svg',
    'church': 'Church.svg',
    'city hall': 'city hall.svg',
    'convenience': 'convenience.svg',
    'drug': 'drug.svg',
    'fire station': 'fire station.svg',
    'gas station': 'gas station.svg',
    'gym': 'gym.svg',
    'hardwarestore': 'hardwarestore.svg',
    'jeep': 'jeep.svg',
    'laundry': 'laundry shop.svg',
    'library': 'library.svg',
    'mall': 'mall.svg',
    'money exchange': 'money exchange.svg',
    'mosque': 'Mosque.svg',
    'park': 'park.svg',
    'playground': 'playground.svg',
    'post office': 'post office.svg',
    'restaurant': 'restaurant.svg',
    'sports complex': 'sports complex.svg',
    'vet clinic': 'vet clinic.svg',
...
const MapController = ({ targetCoords }) => {
    ...
    useEffect(() => {
        ...
        // Removed map.setZoom(16) to keep current zoom level
    }, [targetCoords, map]);
    return null;
};
...
export const VerityMap = () => {
    ...
    // [NEW] State for Traffic Widget visibility
    ...
    const handlePropSelect = (prop) => {
        ...
        setPreciseData({});
        setRouteData(null);
        setShowTraffic(false); // Reset traffic view when changing properties
        setMapTarget([prop.lat, prop.lng]); // Center only on property
    };
        ...
        <div className="relative w-full h-screen bg-gray-100 overflow-hidden">
            <MapContainer center={[10.3157, 123.8854]} zoom={13} style={{ height: '100%', width: '100%' }} zoomControl={false} markerZoomAnimation={false}>
                ...
                {essentialAmenities.map(amen => (
                    <Marker key={`amenity-${amen.id}`} position={[amen.lat, amen.lng]} 
                            ...
                            <div className="text-center min-w-[140px] p-1">
                                ...
                                {preciseData[amen.id] ? (
                                    <div className="pt-2 border-t border-gray-100 flex items-center justify-between gap-2">
                                        <div className="flex flex-col items-center flex-1"><span className="text-xs">🚶</span><span className="text-[10px] font-bold text-gray-700">{preciseData[amen.id].walking} min</span></div>
                                        <div className="w-px h-6 bg-gray-100" />
                                        <div className="flex flex-col items-center flex-1"><span className="text-xs">🚗</span><span className="text-[10px] font-bold text-gray-700">{preciseData[amen.id].driving} min</span></div>
                                    </div>
            ...
            <UnifiedPanel 
                ...
                onClose={() => { setSelectedProp(null); setRouteData(null); }} 
                onAmenitySelect={handleAmenityClick} 
                selectedAmenity={selectedAmenity} 
                preciseData={preciseData}
                // [NEW] Pass traffic toggler to panel
                onTrafficClick={() => setShowTraffic(true)}
            />

            {/* [NEW] Traffic Widget Overlay */}
            {showTraffic && selectedProp && (
                <TrafficWidget 
                    lat={selectedProp.lat} 
                    lng={selectedProp.lng} 
