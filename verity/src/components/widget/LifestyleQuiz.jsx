...
export const LifestyleQuiz = ({ properties, onRecommend, onFilter }) => {
    ...
    const runAnalysis = async () => {
        ...
        try {
            ...
            // MAP DATA: Combine AI text with Real Property Data
            ...
            <div className="p-5 overflow-y-auto custom-scrollbar">
                {step === 'quiz' && (
                    <>
                        ...
                        <button onClick={runAnalysis} disabled={selectedTags.length === 0} className={`w-full py-3 rounded-xl text-xs font-bold flex items-center justify-center gap-2 transition-all shadow-lg ${selectedTags.length > 0 ? 'bg-gray-900 text-white hover:bg-black' : 'bg-gray-100 text-gray-400 cursor-not-allowed'}`}>
                ...
                {step === 'result' && activeMatch && (
                    <div className="animate-in slide-in-from-bottom-4 duration-500 flex flex-col gap-4">
                        ...
                        {/* --- TOP SECTION: THE "ACTIVE" MATCH --- */}
                        <div className="bg-violet-50 border border-violet-100 rounded-xl p-4 relative">
                                ...
                                {matches.map((match, idx) => (
                                    ...
                                    >
                                        <div className={`
                                            ...
                                            ${idx === activeIndex ? 'bg-white text-gray-900' : 'bg-gray-100 text-gray-500'}
                                        `}>
                                            #{idx + 1}
                                        </div>
                                        ...
                                        <div className="flex-1 min-w-0">
                                            ...
                                            <p className={`text-[9px] truncate ${idx === activeIndex ? 'text-gray-300' : 'text-gray-400'}`}>
                                                {match.headline}
                                            </p>
                                        </div>

                                        {idx === activeIndex && <ArrowRight size={12} className="shrink-0" />}
                                    </button>
                                ))}
                            </div>
                        </div>
                        ...
                        <button onClick={handleReset} className="w-full py-3 bg-white border border-gray-200 text-gray-500 rounded-xl text-xs font-bold hover:bg-gray-50 transition flex items-center justify-center gap-2">
