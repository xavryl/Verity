...
export const InquiryModal = ({ isOpen, onClose, property, isLot = false }) => {
  ...
  // Determine what we are inquiring about
  const targetName = isLot ? `Lot #${property?.id}` : property?.name;
  const targetId = property?.id;
  const ownerId = property?.user_id || property?.agent_id; // Handle different DB field names
  ...
  const handleSubmit = async (e) => {
    ...
    try {
        ...
        // Note: Ensure you have an 'inquiries' table in Supabase
        const { error } = await supabase.from('inquiries').insert([{
            property_id: isLot ? null : targetId, // Link to property if it's a property
            lot_id: isLot ? targetId : null,      // Link to lot if it's a lot
            agent_id: ownerId, 
        ...
        // 2. CHECK OWNER PREFERENCE (Mode A vs Mode B)
        let ownerEmail = null;
        if (ownerId) {
        ...
        console.error(err);
        // If table is missing, just show success to user but log error (Hackathon specific)
        if (err.message.includes('relation "inquiries" does not exist')) {
            console.warn("Table 'inquiries' missing. Skipping DB save.");
            setStatus('success'); 
        } else {
        ...
        {status === 'success' ? (
             <div className="p-10 flex flex-col items-center justify-center text-center animate-in fade-in slide-in-from-bottom-4 duration-300">
                ...
                <p className="text-gray-500 text-sm mb-6">
                    Thanks for reaching out about <span className="font-bold">{targetName}</span>. <br/>
            ...
            <>
                <div className="bg-violet-700 p-6 text-white pr-12">
                    ...
                    <p className="text-violet-200 text-sm mt-1">
                        Interested in <span className="font-bold text-white">{targetName || 'this property'}</span>?
                    </p>
                </div>
